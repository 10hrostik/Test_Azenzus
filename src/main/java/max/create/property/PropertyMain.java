...
public class PropertyMain  {
	...
	public void inputAllData() throws InterruptedException, IOException {
		if (!checkIfExist()) {
			...
			XlsxReaderForProperty.setResult("Already exist");
			try {
				XlsxReaderForProperty.setID(getId());
			}
			catch (Exception ignored) {
    ...
    public int getId() {
		return Integer.parseInt(Az.findElement(By.xpath("//tr[@role='listitem']/td[2]/div[text()='"+XlsxReaderForProperty.getName()+"']/ancestor::tr/td[1]/div")).getText());
    }
}